...
class ROS_Classify:
    ...
    def __init__(self,model,decoupled=False,plot=False):
        ...
        self.sub = ApproximateTimeSynchronizer([self.image_rect_color_left,self.image_rect_color_right,self.lidar_sub], queue_size = 50, slop = 0.05)
    ...
    def image_callback(self,image_left,image_right,lidar_msg):
        ...
        cv_image= np.concatenate((cv_image,limited_ranges),axis = -1)
        cv_image2= np.concatenate((cv_image2,limited_ranges),axis = -1)
